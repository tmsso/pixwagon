...
// Pass 02: adds ghost states for the uncommitted placement preview (data-ghost / data-ghost2)
// and explicit per-cell seat fills for own-board comparison (data-seatcells).
const PACK = {
  tram: {
    name: 'Tram',
    palette: ['#d4483b', '#e8f1f5', '#2c3038', '#8a9199'],
    rows: ['.....3......', '.....3......', '.0000000000.', '.0111011110.', '.0111011110.', '.0000000000.', '.0000000000.', '..2......2..'],
  },
  sailboat: {
    name: 'Sailboat',
    palette: ['#8a5a34', '#f3ede2', '#4a3b2a', '#3f8fbf'],
    rows: ['.....2.....', '...112.....', '..1112.....', '.11112.....', '111112.....', '.....2.....', '.0000000...', '..00000....', '.333333333.'],
  },
  balloon: {
    name: 'Hot-air balloon',
    palette: ['#e0703a', '#f5c451', '#7a5230', '#5c6670'],
    rows: ['...000...', '.0000000.', '000000000', '011111110', '011111110', '000000000', '.0000000.', '..00000..', '...3.3...', '..22222..', '..22222..'],
  },
};
...
function render(el) {
  ...
  const ghost = parseCoords(el.dataset.ghost);
  const ghost2 = parseCoords(el.dataset.ghost2);
  ...
  pic.rows.forEach((row, y) => {
    Array.from(row).forEach((char, x) => {
      ...
      if (mode === 'target') {
        ...
        if (!blank) { span.style.outline = '1px solid var(--cell-fillable)'; span.style.outlineOffset = '-1px'; }
      } else {
        // board: live play state
        if (blank) { span.classList.add('c-blank'); }
        else if (inv.has(key)) { span.classList.add('c-inv'); }
        else if (ghost.has(key)) { span.classList.add('c-ghost'); }
        else if (ghost2.has(key)) { span.classList.add('c-ghost2'); }
        else if (cand.has(key)) { span.classList.add('c-cand'); }
        else if (lock.has(key)) { span.classList.add('c-locked'); }
        else if (((x * 73 + y * 151 + 17) % 97) / 97 < progress) {
          span.classList.add('c-done', 'p' + seats[(x + y) % seats.length]);
        } else { span.classList.add('c-fill-empty'); }
      }
      el.appendChild(span);
    });
  });
