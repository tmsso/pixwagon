# Delivery log — Pixwagon

**What this file is.** Until 2026-09-16 this was `ROADMAP.md`, and it had grown into a full delivery narrative — every phase's progress notes, dated corrections and verification records, interleaved with the plan. The repo review that day (`docs/review-2026-09-16.md`) split the two: `ROADMAP.md` is now plan-only, and this file preserves the record **verbatim** from that date (below the rule). Phase numbers are shared between the two files, so a `ROADMAP.md Phase N` reference in code resolves in either.

**How to use it.** Append dated entries under "Later entries" at the bottom when a batch ships; keep the plan in `ROADMAP.md` short and put the narrative here. Do not edit the frozen section.

---

# ROADMAP.md — Pixwagon build plan (frozen 2026-09-16)
...
### 2026-09-24 — item 4 deployed to Pages and verified live

`pnpm --filter @pixwagon/web build` (production; bundle points at `pixwagon-app.tmsso.workers.dev`) then `wrangler pages deploy` (workspace-pinned 4.116) → `pixwagon.pages.dev`. Live check with two headless Chromium instances against `https://pixwagon.pages.dev` and the live worker, the second one routed through a local CONNECT proxy so its connection could be cut for real: create room → join → "Players · 2 of 6", seat order Alex (host) then Sam → identical round-1 and round-2 offers on both → all of B's proxy tunnels destroyed and new ones refused for 10 s: B shows "Reconnecting…", retries at ~0.5/1/2/4 s (1006 each), A's presence drops Sam → proxy restored: B's `welcome` carries the same `playerId` as before, round 2, identical offer, no page reload; A sees Sam again. Two earlier live attempts were discarded as vacuous: closing the socket from inside the page left the client waiting on a close handshake the worker never answers (see ROADMAP Phase 4 note), so its "Connected" pill was stale.

### 2026-09-24 — Phase 5 items 1–2: server boards, round lifecycle, protocol (merged, not deployed)

Room storage gains `status`, `packId`, `pictureId`, `roundBudget`, `boards`, `acted`; pure `roomState.ts` functions `startGame`, `ensureBoard`, `submitFill`, `submitPass`, `awaitingPlayers`, `closeRoundIfDone`, `normaliseRoom`, with `room.ts` as glue: the host's `request-roll` starts the game, `fill`/`pass` go through the referee, and every fill, pass and disconnect tries to close the round. Protocol typed per the contract's "Decided in Phase 5". game-core exports `fillCells` for the delta drift guard (and the client, next).

Observed against `wrangler dev` with raw WebSocket clients: start → both get the same snapshot (`playing`, a picture, a budget, two boards); a tampered fill with face-correct blob sizes on blank cells → `fill-rejected: cell-not-fillable` to the sender, no `delta` to anyone; a legal fill → `fill-accepted` plus an identical `delta` to both; a second action → `already-acted`; the round closes only after the other player passes → `round-result` + the next roll, identical on both; a mid-game `request-roll` → `game-in-progress`; the last un-acted player disconnecting closes the round for the other; the rejoiner gets the same id and their board back; a room stored by the Phase 4 code loads as a lobby and starts cleanly.

Two server fixes landed first as their own PRs, found while verifying: #26 (the worker echoes a client-initiated close; before it the client waited ~10 s for a 1006) and #27 (a regression #26 introduced: a send to the just-closed socket threw and aborted the whole broadcast, so a leaver's presence never reached the room; `fanout.ts` now guards each send). Neither is deployed; they must go out together.

Not deployed, by the owner's decision for this batch: the live client cannot fill or pass until item 3, so a Phase 5 worker would stall live rooms after round 1. The live worker stays on PR #24's code for the phone test.
