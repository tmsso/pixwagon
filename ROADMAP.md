...
| 5     | Same-board multiplayer                | next      | —           | started under the §8 device-check exception (2026-09-24)  |
...
Found 2026-09-24 while verifying item 4, not yet fixed live: (1) the worker never answered a _client-initiated_ WebSocket close (the client waited ~10 s and got 1006); harmless for a real network drop, which has no handshake. **Fixed in code by PR #26** (`Room.webSocketClose` echoes the close; local before/after 10 059 ms/1006 → 57 ms/1000) but **not deployed** — a worker redeploy awaits the owner's go-ahead. #26 on its own regressed `main`: broadcasting to the just-closed socket threw and aborted the fan-out, so a leaver's presence update never reached the room; fixed by the follow-up PR that guards each send (`apps/server/src/fanout.ts`). Deploy the two together, never #26 alone. (2) A dropped player vanishes from everyone's presence until they rejoin, where pass 02 `12c` wants a dimmed "away" chip — needs a `connected` flag on `PlayerPresence`; natural to add with Phase 5's per-player boards, since a player's board must outlive their socket.
