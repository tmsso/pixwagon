...
1. ✅ **Done 2026-09-17** (during the item-2 batch, owner authorized Claude to run the deploy itself rather than requiring an owner-run step here). Redeployed the worker so the live room server matches `main`: `pnpm exec wrangler deploy` from `apps/server/`; `curl https://pixwagon-app.tmsso.workers.dev/api/config` → `{"protocolVersion":1,"maxPlayers":6}`.
   _Before steps 2 and 4 on a phone that has opened the site before:_ the installed service worker keeps serving the old build until its "update available" banner on Home is accepted — accept it first, or the phone tests the pre-Lobby build.
...
4. ✅ **Lobby and networked Game screen, provisional design (pass 02 `02a–02e`, `12a–12c`).** Delivered 2026-09-24 — see the Decided line below for the choices it made; original bullet kept: Lobby: create room (`POST /api/room`), join by code (`RoomCodeInput`), player chips with seat colour + hatch, host sees "Start round". Game: `HudFrame` gets its `connection` pill; `RollControl` shows the server's roll; **no filling yet** — the sheet stays disabled with copy "Placing pieces arrives with the next update". Keep `/r/solo` on `soloGame` and route `/r/:code` to `roomGame` when `code !== 'solo'`. Known-and-deliberate from item 3: `applyServerMessage` drops a `roll` that arrives before the first `state` (a late joiner racing the host's broadcast) — self-corrects on the next `state`/resync, but the Game screen will show a blank `RollControl` for that gap; worth a look if it's more than momentary in the two-device test. **Done means:** `scripts/check-screens.tsx` covers Lobby and the networked Game with a fixture snapshot; `pnpm verify` green.
5. ✅ **Origin config (D4)** and a `pnpm dev:server` script (`wrangler dev`) so a local two-tab test works without deploying. Delivered 2026-09-24, ahead of item 4 because item 4 cannot connect anywhere without it. `apps/web/src/net/roomOrigin.ts` resolves `VITE_ROOM_ORIGIN` → `localhost:8787` under `pnpm dev` → the live worker in a production build, and swaps `http(s)` for `ws(s)` on the socket URL. Found while doing it: D4's "no CORS needed" holds for the WebSocket only — `POST /api/room` is a plain `fetch`, so the worker now sends `Access-Control-Allow-Origin` for an allowlist (`apps/server/src/cors.ts`); worker redeployed 2026-09-24 (version `f90592b5`); live `POST /api/room` with `Origin: https://pixwagon.pages.dev` now returns `access-control-allow-origin: https://pixwagon.pages.dev`, and a foreign origin gets none.

Decided in this phase so far: host-only `request-roll` (2026-09-09); `MAX_PLAYERS` lives in `protocol` (2026-09-09); room storage persists after the last socket closes, so rejoining a code resumes rather than restarts (2026-09-09). Worker HTTP endpoints send CORS headers for an explicit origin allowlist — `pixwagon.pages.dev`, its preview subdomains, local `vite` ports — not `*` (2026-09-24). Item 4 (2026-09-24): the waiting room lives at the room's own URL `/r/CODE`, not `/lobby`, so a copied link lands everyone in one place — `roomPhase` shows it until the first roll, the networked game after; starting needs two players (pass 02 `02c`); `02b`'s invalid state fires for a malformed code only — a well-formed code nobody created opens a fresh empty room, since a room code is a Durable Object name with no registry; the display name lives in `localStorage` (default "Guest"); `room-full` and `protocol-version-mismatch` are terminal — the store stops reconnecting instead of retrying into a full room; the room-code alphabet moved to `protocol` (`ROOM_CODE_ALPHABET`) — the design's sample code "PIXW" contains an I, which is never issued.

Found 2026-09-24 while verifying item 4, not yet fixed live: (1) the worker never answered a _client-initiated_ WebSocket close (the client waited ~10 s and got 1006); harmless for a real network drop, which has no handshake. **Fixed in code by PR #26** (`Room.webSocketClose` echoes the close; local before/after 10 059 ms/1006 → 57 ms/1000) but **not deployed**. #26 on its own regressed `main`: broadcasting to the just-closed socket threw and aborted the fan-out, so a leaver's presence update never reached the room; fixed by the follow-up PR that guards each send (`apps/server/src/fanout.ts`). Deploy the two together, never #26 alone — and since #28 now sits on `main` behind them, they ship bundled with Phase 5 (recommended: with item 3; the live Phase 4 worker has neither the regression nor any need for the echo, since an aeroplane-mode drop has no close handshake), unless the owner asks for a cherry-pick deploy of just the two fixes onto the PR #24 base. (2) A dropped player vanishes from everyone's presence until they rejoin, where pass 02 `12c` wants a dimmed "away" chip — needs a `connected` flag on `PlayerPresence`; still open after Phase 5 item 1 (boards now outlive sockets in the snapshot, but presence still lists live sockets only) — a small protocol addition for item 3's client work.
