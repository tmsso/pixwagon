...
Live: web app `https://pixwagon.pages.dev` (Pages, deploys PR #18 build) · room worker `https://pixwagon-app.tmsso.workers.dev` — **redeployed 2026-09-24** (`wrangler deploy`, version `f90592b5`), running `main` as of PR #24 (protocol/room-state, rejoin identity, CORS allowlist). Live-verified: `/api/config` → `{maxPlayers:6}`; a real WebSocket `join` returns `welcome.rejoinToken`; a second `join` resending that token on a fresh connection reclaims the same `playerId` and `seatIndex` (name updates, as designed).
