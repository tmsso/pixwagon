...
- **Verified**: `pnpm verify` green (187 tests, +23: protocol room-state shapes, `roomState.ts`, the drift guard, the web seat-count guard). Live: after deploy, `/api/config` still returns `{ maxPlayers: 6 }` (the `MAX_SEATS`→`MAX_PLAYERS` swap on a deployed endpoint). The two-device presence/roll check and the network-kill resync are **not verifiable this session** (no second device; resync is the deferred client half) — they stay open on the Accept line.
