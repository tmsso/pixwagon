...
1. **`apps/web/src/net/roomConnection.ts` — a transport class with no React.** Owns one `WebSocket`, encodes/decodes via `@pixwagon/protocol`, exposes `send()`, `close()` and an `onMessage` callback. Reconnect with capped exponential backoff (0.5 s → 8 s, jittered) on any close that was not a deliberate `leave`. Constructed with a `WebSocket`-like factory so tests inject a fake. **Done means:** unit tests for backoff timing, message decode errors surfacing as a typed event, and "deliberate close does not reconnect".
